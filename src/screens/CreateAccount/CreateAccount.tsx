...
const CreateAccount: FunctionComponent<CreateAccountProps> = ({}) => {
    ...
    const validation = Yup.object({
        ...
        confirmPassword: Yup.string().required("Veuillez confirmer votre mot de passe").oneOf([Yup.ref("password")], "Les mots de passe ne correspondent pas")
    }).required();
...
return (
    <SafeAreaView>
        <ScrollView>
            <Controller control={control} name='pseudo' render={({field: {onChange, value, onBlur}, fieldState: {error}}) => (
                <CustomTextFiled 
                    value={value}
                    placeholder="Pseudo"
            ...
            <CustomButton type='ACTIVE_PRIMARY' onPress={handleSubmit(onSignUp)} text='Créer un compte' />
